...
//        mRefreshImageView.setBackgroundResource(R.mipmap.cats);//换图片的可以这里换，然后在onlayout里面设置高度
    ...
    private void ensureTarget() {
        if (mTarget != null)
            return;
        if (getChildCount() > 0) {
            //0是刷新的那个imageview，1开始是在pulltorefreshview按顺序的控件
            //mTarget是需要滑动的那个控件，mOther是其他的控件
            for (int i = 0; i < getChildCount(); i++) {
                View child = getChildAt(i);
                    if (child != mRefreshImageView) {
                        mTarget = child;
                        scrollTarget =((ViewGroup) child).getChildAt(0);
                    }
            }
        }
    }
    ...
    private boolean canChildScrollUp() {
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (scrollTarget instanceof AbsListView) {
                final AbsListView absListView = (AbsListView) scrollTarget;
                return absListView.getChildCount() > 0
                        && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(0)
                        .getTop() < absListView.getPaddingTop());
            } else {
                return scrollTarget.getScrollY() > 0;
            }
        } else {
            return ViewCompat.canScrollVertically(scrollTarget, -1);
        }
    }
...
//        Math.round((float) DRAG_MAX_DISTANCE * density);//高度
        mRefreshImageView.layout(left, top, left + width - right, top + height - bottom);
//        BaseUtil.ShowLog("123",left+"    "+top+"     "+(left + width - right)+"      "+(top + height - bottom));
    }
